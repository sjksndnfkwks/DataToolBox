#' Function to forward select variables and do regression based on selected variables.
#'
#' This function takes a data set, a response variable and expected number of regressors as input
#' and returns the selected variable based by forward selection and AIC.
...
#' @return The selected variables, summary of such regression model if you set the summary TRUE
#' @examples
#' # The following sample data set includes socio-economic information with suicide rates by country in 2011.
#' data("SuicideData")
#' forward_selection_regression(SuicideData, "suicides.100k.pop", 3)
#' forward_selection_regression(SuicideData, "suicides.100k.pop", 4)
#' forward_selection_regression(SuicideData, "suicides.100k.pop", 3, summary = TRUE)
#'
#' @export

forward_selection_regression = function(data, response, expected_number_of_regressor, summary = FALSE){
  ...
  # Check if the expected number of regressors does not exceed the number of available predictors
  ...
  # We get the all predictor variable names
  regressor_set = colnames(data)[colnames(data) != response]
  selected_regressors = character()
  best_aic = Inf

  # Forward selection of regressor variables
  while (length(selected_regressors) < expected_number_of_regressor) {
    candidate_aic = numeric(length(regressor_set))
    # A for loop to through all
    for (i in seq_along(regressor_set)) {
      predictors = c(selected_regressors, regressor_set[i])
      current_formula = as.formula(paste(response, "~", paste(predictors, collapse = "+")))
      model = lm(formula = current_formula, data = data)
      candidate_aic[i] = AIC(model)  # get aic
    }

    # We want the lowest aic.
    best_candidate_index = which.min(candidate_aic)
    best_candidate_aic = candidate_aic[best_candidate_index]
    # If the AIC value of the best candidate model is lower than the current best one, then we add the variable to the selected regressors set
    if (best_candidate_aic < best_aic) {
      best_candidate_regressor = regressor_set[best_candidate_index]
      selected_regressors = c(selected_regressors, best_candidate_regressor)
      regressor_set = regressor_set[-best_candidate_index]
  ...
  # If the parameter summary is TRUE,return the summary of the final model
  if (summary) {
